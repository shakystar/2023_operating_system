...
#define SCHED_LOCK_PW 2020003309 

struct {
  struct spinlock lock;
  struct proc proc[NPROC]; // Num of Process
  struct mlfq* mlfq; // multi level feedback queue // ready queue와 같은 역할
  struct proc* lockproc; // scheduler lock
} ptable;
...
found:
  ...
  p->level = 0;
  p->priority = 3;
  p->time_quantum = 4; // 초기값 4인 이유 : 감소 형식으로 쓰면 Queue에 배치할 때 2n+4 연산을 안할 수 있음.
  p->next = 0;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // scheduler lock 할당된 proc 존재 시, runnable이면 cpu 할당 runnable하지 않을 경우 cpu 양보
...
boosting(void)
{
  ...
  // scheduler lock에 할당된 process가 있을 경우
  if (ptable.lockproc)
  {
    // 다음 차례 process를 깨움
